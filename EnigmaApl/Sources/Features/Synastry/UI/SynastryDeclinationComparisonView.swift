// SynastryDeclinationComparisonView.swift
...
/// Two parallel tables for a synastry comparison: each table lists, for one person's
/// factors, the parallels and contra-parallels formed to the other person's factors
/// (parallel glyph + orb). Factors and orbs follow the active configuration.
struct SynastryDeclinationComparisonView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if sideBySide {
                HStack(alignment: .top, spacing: sectionSpacing) {
                    parallelSection(ownerName: first.name, matches: matches)
                        .frame(minWidth: minTableWidth, alignment: .leading)
                    parallelSection(ownerName: second.name, matches: matches.map { $0.reversed })
                        .frame(minWidth: minTableWidth, alignment: .leading)
                }
            } else {
        ...
        .sheet(isPresented: $showHelp) {
            WheelHelpSheet(helpText: t(SynastryKeys.helpDeclinationComparison))
        }
    }
    ...
    @ViewBuilder
    private func parallelSection(ownerName: String, matches: [ParallelRow], scrollable: Bool = true) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(ownerName)
                .font(.title3.weight(.semibold))

            if matches.isEmpty {
                Text(t(SynastryKeys.declinationComparisonNoMatches))
    ...
    @ViewBuilder
    private func table(matches: [ParallelRow], scrollable: Bool) -> some View {
        if scrollable {
            GroupBox {
    ...
    @ViewBuilder
    private func tableContent(matches: [ParallelRow]) -> some View {
        VStack(spacing: 0) {
            HStack(spacing: 8) {
                Text(t(SynastryKeys.declinationComparisonColRadix))
                    .frame(width: glyphW, alignment: .center)
                Spacer().frame(width: glyphW)
                Text(t(SynastryKeys.declinationComparisonColPartner))
                    .frame(width: glyphW, alignment: .center)
                Text(t(SynastryKeys.declinationComparisonColOrb))
    ...
    @ViewBuilder
    private func rowView(_ row: ParallelRow, index: Int) -> some View {
        HStack(spacing: 8) {
            ...
            Text(row.isContraParallel ? "\u{F010}" : "\u{F000}")
                .font(.custom("EnigmaAstrology3", size: 18))
                .frame(width: glyphW, alignment: .center)
    ...
    private struct ParallelRow {
        ...
        var reversed: ParallelRow {
            ParallelRow(ownFactor: otherFactor, isContraParallel: isContraParallel, otherFactor: ownFactor, orb: orb)
        }
    }

    private var matches: [ParallelRow] {
        guard let config = activeConfigs.first else { return [] }
        let found = SynastryDeclinationOrchestrator.calculate(
            chartA: first.chart,
            chartB: second.chart,
        ...
        return found.map {
            ParallelRow(ownFactor: $0.factor1, isContraParallel: $0.isContraParallel, otherFactor: $0.factor2, orb: $0.actualOrb)
        }
    }
    ...
    private var exportContent: some View {
        VStack(alignment: .leading, spacing: 20) {
            Text(t(SynastryKeys.resultsTitleDeclinationComparison))
                .font(.title2.weight(.semibold))

            HStack(alignment: .top, spacing: sectionSpacing) {
                parallelSection(ownerName: first.name, matches: matches, scrollable: false)
                parallelSection(ownerName: second.name, matches: matches.map { $0.reversed }, scrollable: false)
            }
        }
        .padding(24)
    ...
    private func exportPDF() {
        ...
        panel.nameFieldStringValue = "Synastry declinations.pdf"
