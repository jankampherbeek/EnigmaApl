// PreNatalAspectSelectionSheet.swift
...
struct PreNatalAspectSelectionSheet: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Text(t(PreNatalKeys.aspectSelTitle))
                .font(.headline)
                .padding()
            ...
            HStack {
                ...
                Button(t(PreNatalKeys.selectionDone)) {
                    model.selectedAspects = Aspects.allCases.filter { selected.contains($0) }
                    dismiss()
                }
