...
func drawPlanetConnectLines(_ ctx: inout GraphicsContext, center: CGPoint, outerRadius: Double, data: WheelPlotData) {
    ...
    for item in data.planetItems {
        ...
        ctx.stroke(path, with: .color(WheelColors.planetConnectLine.opacity(WheelMetrics.connectLineOpacity)), lineWidth: stroke)
    }
}

func drawPlanetGlyphs(_ ctx: inout GraphicsContext, center: CGPoint, outerRadius: Double, data: WheelPlotData) {
    ...
    for item in data.planetItems {
        ...
        let text = Text(item.glyph).font(.custom("EnigmaAstrology2", size: fontSize))
                    .foregroundColor(WheelColors.planetGlyph)
        ctx.draw(ctx.resolve(text), at: pt, anchor: .center)
    }
}

func drawPlanetTexts(_ ctx: inout GraphicsContext, center: CGPoint, outerRadius: Double, data: WheelPlotData) {
    ...
    for item in data.planetItems {
        ...
        let text = Text(item.positionText).font(.system(size: fontSize)).foregroundColor(WheelColors.planetText)
