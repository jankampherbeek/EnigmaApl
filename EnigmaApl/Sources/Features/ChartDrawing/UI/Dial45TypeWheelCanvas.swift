// Dial45TypeWheelCanvas.swift
// EnigmaApl is open source. For more information see se_license.html and License, both at the root of the application.
// Created by Jan Kampherbeek 2026
//
// Canvas for the Ebertin-style 45° dial wheel.
//
// Layout from centre outward:
//   1. Planet area    (0 … fHalfDegInner)          — glyphs, degree texts, centre cross
//   2. 0.5° ring      (fHalfDegInner … f1DegInner) — 90 ticks: short (0.5°), medium (1°),
//                                                     long (5°, extends inward past fHalfDegInner)
//   3. 1° ring        (f1DegInner … fLabelInner)   — 45 uniform ticks (one per dial degree)
//   4. Label ring     (fLabelInner … fLabelOuter)  — degree labels every 5° (0, 5, 10 … 40)
//
// Visual angle = (ecliptic longitude mod 45) × 8.
...
private let fTickHalfDeg:     Double = 0.012   // 0.5° tick
private let fTickWholeDeg:    Double = 0.030   // 1° tick
private let fTick5Deg:        Double = 0.080   // 5° tick: spans ring + extension inside
...
struct Dial45TypeWheelCanvas: View {
...
private func drawDial45Background(_ ctx: inout GraphicsContext, center: CGPoint, R: Double,
                                   theme: WheelTheme) {
...
private func drawDial45RingStrokes(_ ctx: inout GraphicsContext, center: CGPoint, R: Double,
                                    theme: WheelTheme) {
...
// MARK: - 1° ring: 45 uniform ticks (one per dial degree, each spanning the full ring width)

private func drawDial451DegTicks(_ ctx: inout GraphicsContext, center: CGPoint, R: Double,
                                  theme: WheelTheme) {
    ...
    for i in 0..<45 {
        let angle = Double(i) * 8.0   // 45 × 8° = 360°
...
// MARK: - 0.5° ring: 90 ticks with variable lengths; 5° ticks extend into planet area

private func drawDial45HalfDegTicks(_ ctx: inout GraphicsContext, center: CGPoint, R: Double,
                                     theme: WheelTheme) {
...
// MARK: - Degree labels (0, 5, 10 … 40) in the label ring

private func drawDial45DegreeLabels(_ ctx: inout GraphicsContext, center: CGPoint, R: Double,
                                     theme: WheelTheme) {
    ...
    for i in 0..<9 {
        let dialDeg = i * 5
        let angle   = Double(dialDeg) * 8.0   // dial degrees → visual angle
...
private func drawDial45CenterCross(_ ctx: inout GraphicsContext, center: CGPoint, R: Double,
                                    theme: WheelTheme) {
...
private func drawDial45ConnectLines(_ ctx: inout GraphicsContext, center: CGPoint,
...
private func drawDial45PlanetGlyphs(_ ctx: inout GraphicsContext, center: CGPoint,
                                     R: Double, data: WheelPlotData, theme: WheelTheme) {
...
private func drawDial45PlanetTexts(_ ctx: inout GraphicsContext, center: CGPoint,
                                    R: Double, data: WheelPlotData, theme: WheelTheme) {
    let r        = R * fPlanetText
    ...
    for item in data.planetItems {
        ...
        let text = Text(item.positionText)
            ...
            .foregroundColor(theme.planetText)
                 + Text(" " + signGlyph45(for: item.eclipticLongitude))
            .font(.custom("EnigmaAstrology2", size: fontSize))
            .foregroundColor(theme.planetText)
        drawDial45RotatedText(&ctx, text: text, at: pt, angle: item.plotAngle)
    }
}

// MARK: - Helpers

private func signGlyph45(for longitude: Double) -> String {
    let signIndex = Int(longitude / 30.0) % 12
    let sign = Signs(rawValue: signIndex + 1) ?? .Aries
    return GlyphSelector.getGlyphForSign(sign)
}

private func drawDial45RotatedText(_ ctx: inout GraphicsContext, text: Text,
