...
struct OccupiedMidpointsView: View {
    let dialType: MidpointDialType
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                Text(t(MidpointsKeys.occupiedTitle))
                    .font(.title2.weight(.semibold))

                if matches.isEmpty {
                    Text(t(MidpointsKeys.occupiedNoMidpoints))
