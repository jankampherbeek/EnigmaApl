...
public struct ElementsCalc {
    ...
    public static func calculateElementsFactors(
        ...
        for factor in request.FactorsToUse {
            ...
            // calculte position 0.5 day before and after the calculated time, to define the speed
            let previousPosition = calculatePosition(
                factor: factor,
                julianDay: julianDayPrevious,
                observerPosition: observerPosition,
                calcHelioPos: &calcHelioPos
            )
            let nextPosition = calculatePosition(
                factor: factor,
                julianDay: julianDayNext,
                observerPosition: observerPosition,
                calcHelioPos: &calcHelioPos
            )
            let longSpeed = nextPosition.longitude - previousPosition.longitude
            let latSpeed = nextPosition.latitude - previousPosition.latitude
            let radvSpeed = nextPosition.distance - previousPosition.distance
            ...
            let (previousRightAscension, previousDeclination) = seWrapper.eclipticToEquatorial(
                eclipticCoordinates: [previousPosition.longitude, previousPosition.latitude],
                obliquity: obliquity)
            let (nextRightAscension, nextDeclination) = seWrapper.eclipticToEquatorial(
                eclipticCoordinates: [nextPosition.longitude, nextPosition.latitude],
                obliquity: obliquity)
